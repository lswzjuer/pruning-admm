# -*- coding: utf-8 -*-
# @Author: liusongwei
# @Date:   2020-09-16 21:45:26
# @Last Modified by:   liusongwei
# @Last Modified time: 2020-09-18 02:14:28
import sys
import torch
...
def getParams(model):
    ...
    MB
    '''
    total_params=np.sum(np.prod(v.size()) for v in model.parameters())
    return params_to_string(total_params)



def getFlops(model, input_shape=(3, 224, 224)):
    '''
    Only the convolution layer and the full connection layer are calculated
    MB
    '''
    list_conv = []
    ...
    return flops_to_string(total_flops,None)
...
def get_model_complexity_info(model, input_res,
    ...
    '''
    A more comprehensive analysis of both parameters and computations
    '''
...
def compute_average_flops_cost(self):
    ...
    for module in self.modules():
        ...
        # else:
        #     print("there is not support moudle: {}".format(type(module).__name__))
...
def bn_flops_counter_hook_gb(module, input, output):
    module.affine
    input = input[0]
    group=module.num_groups
    batch_flops = np.prod(input.shape)
    if module.affine:
        batch_flops *= 2
    module.__flops__ += (int(batch_flops)/group)
...
def deconv_flops_counter_hook(conv_module, input, output):
    ...
    kernel_height,kernel_width=dilation*(kernel_height-1)+1,dilation*(kernel_width-1)+1
...
def conv_flops_counter_hook(conv_module, input, output):
    ...
    kernel_dims=[  dilation[0]*(i-1)+1 for i in kernel_dims]
    in_channels = conv_module.in_channels
    out_channels = conv_module.out_channels
    groups = conv_module.groups

    filters_per_channel = out_channels // groups
    conv_per_position_flops = np.prod(kernel_dims,dtype=np.float32)* in_channels * filters_per_channel
...
MODULES_MAPPING = {
    ...
    torch.nn.GroupNorm:bn_flops_counter_hook_gb,
