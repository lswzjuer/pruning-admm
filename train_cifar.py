...
def main():
    ...
    # do mask 
    model_stage1 = pruner1.wrapper_model()
    evaluation_result = evaluator(model_stage1)
    ...
    # finetune
    optimizer1,scheduler1 = get_optimizer_scheduler(args,model_stage1)

    best_acc = 0
    for epoch in range(args.fintune_epochs):
        train(args, model_stage1, args.device, trainLoader, criterion, optimizer1, epoch, logger=logger)
        scheduler1.step()
        acc = evaluator(model_stage1)
        if acc > best_acc:
            best_acc = acc
            torch.save(model_stage1.state_dict(), os.path.join(modelDir, 'model_masked_finetune.pth'))
    model_stage1.load_state_dict(torch.load(os.path.join(modelDir, 'model_masked_finetune.pth')))
    evaluation_result = evaluator(model_stage1)
    logger.info('Evaluation result (fine tuned): {} and {}'.format(best_acc,evaluation_result))
